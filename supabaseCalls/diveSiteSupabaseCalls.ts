...
export const getDiveSiteById = async (id: number | string) => {
  const { data, error } = await supabase.rpc("get_dive_site_by_id_with_images", {
    idnum: id
  });

  if (error) {
    console.log("Error in getDiveSiteById:", error);
    return null;
  }

  return data; // This will be the single object directly
};
