...
type MapContextType = {
  ...
  setBoundaries: React.Dispatch<
    React.SetStateAction<google.maps.LatLngBounds | null>
  >;
  ...
  setDraggablePoint: React.Dispatch<
...
export const MapContextProvider = ({ children }: any) => {
  ...
  const [boundaries, setBoundaries] = useState<google.maps.LatLngBounds | null>(
    null
  );
  const [initialPoint, setInitialPoint] = useState<number[]>([
    49.316666, -123.066666,
  ]);
  const [mapConfig, setMapConfig] = useState<number>(0);
  const [draggablePoint, setDraggablePoint] =
