...
export const StyledButton = styled.View`
  ...
  background-color: ${({ alt }) =>
    alt ? colors.themeWhite : colors.primaryBlue};
  border-radius: ${({ round }) =>
    round ? `${moderateScale(30)}px` : `${moderateScale(10)}px`};
...
export const StyledButtonText = styled.Text`
  font-size: ${({ size }) =>
    size === "thin"
      ? `${moderateScale(fontSizes.SmallText)}px`
      : `${moderateScale(fontSizes.StandardText)}px`};
  font-family: ${({ size }) =>
    size === "thin" ? activeFonts.Medium : activeFonts.Regular};
...
export const IconWrapperLeft = styled.View`
  width: ${({ size }) =>
    size === "thin" ? `${moderateScale(18)}px` : `${moderateScale(24)}px`};
